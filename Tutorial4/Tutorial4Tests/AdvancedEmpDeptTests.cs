...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { e, d }).SelectMany(x => grades.Where(g => x.e.Sal >= g.Losal && x.e.Sal <= g.Hisal).Select(g => new { x.e.EName, x.d.DName, g.Grade })).ToList(); 
